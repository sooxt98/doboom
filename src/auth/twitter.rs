...
use hyper::header::{Accept, Bearer, Basic, Headers, Authorization, ContentType, UserAgent, qitem};
...
pub fn auth(code: String) -> Result<String, String> {
    ...
    let credential = _credential.as_bytes().to_base64(STANDARD);
    ...
    let worker = client.request(code_req).and_then(|code_result| {
        ...
        code_result.body().fold(Vec::new(), |mut v, chunk| {
            ...
            account_req.headers_mut().set(UserAgent(String::from("Doboom")));
